...
export default {
  ...
  methods: {
    ...
    calculateAnswer(answer, correct, qId) {
      if (answer == correct) {
        this.$store.commit('spend/setScore', [qId.toString(), "right", answer])
      } else { this.$store.commit('spend/setScore', [qId.toString(), "'wrong'", answer]) }
    },
    arraysMatch(arr1, arr2, qId) {
      if (arr1.length !== arr2.length) {
        this.$store.commit('spend/setScore', [qId.toString(), "'wrong'", arr1])
        return false
      }
      ...
      this.$store.commit('spend/setScore', [qId.toString(), "right", arr1])
    }
  },
  computed: {
      allDone(){
